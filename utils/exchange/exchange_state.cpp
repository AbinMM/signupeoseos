...
asset exchange_state::convert( asset from, symbol_type to ) {
    ...
    // Use fast alg
    if(base.weight == quote.weight){
        const double QUOTE_AMOUNT = double(quote.balance.amount);
        const double BASE_AMOUNT = double(base.balance.amount);
        const double IN  = double(from.amount);

        // buying token
        if(sell_symbol == base_symbol && to == quote_symbol){
            return asset(int64_t((IN*QUOTE_AMOUNT) / (IN+BASE_AMOUNT)), to);

            // selling token
        }else if (sell_symbol == quote_symbol && to == base_symbol){
            return asset(int64_t((IN*BASE_AMOUNT)/(QUOTE_AMOUNT+IN)), to);
        }
    }
